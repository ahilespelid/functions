...
if (!function_exists('array_unflatten')){function array_unflatten(array $flatArray): array {
        $result = []; foreach($flatArray as $key => $value){if(!is_string($key)) {continue;}
            $keys = explode('.', $key); $current = &$result;
            foreach($keys as $index => $segment){
                if($index === count($keys) - 1){$current[$segment] = $value;}else{
...
if(!function_exists('array_keys_recursive_filter')){function array_keys_recursive_filter(array $array, array $keep_keys): array {
    return array_map(fn($i) => is_array($i) ? filter_array_keys_recursive($i, $keep_keys) : $i, array_intersect_key($array, array_flip($keep_keys)));}}
//------------------------------------------------------------------------------FILE-SYSTEM------------------------------------------------------------------------------------------------------------------------------//  
